...
<script>
  ...
  export default {
    ...
    data() {
      ...
      drawCategory() {
        ...
        this.charts.on('click', function (params) {
          if (params.componentType == "xAxis") {
            ...
            eventbus.$emit('SendPersonalMonth', month)
          }
        });
      ...
      /*根据年份显示柱状图*/
      GetYear() {
        eventbus.$emit('SendPersonalYear', this.value3)

        this.$request.post('/api/PersonalData/PersonalRecord', {
          'Year': this.value3,
          "FamilyId": sessionStorage.getItem('familyid')
        }).then((res) => {
          console.log(res);
          this.zc = res['PersonalSpend'];
          this.sr = res['PersonalIncome'];
        }).catch()
