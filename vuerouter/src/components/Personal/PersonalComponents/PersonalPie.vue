<template>
    <div class="PieDiv">
        <div class="PieTypeS">
            <div class="PieTitleS">{{year+'年'+month+'月'}}个人支出</div>
            <div class="PieTypeSTx">
                <div id="pieReportS" class="Pie"></div>
            </div>
        </div>
        <div class="PieTypeC">
            <div class="PieTitleC">{{year+'年'+month+'月'}}个人收入</div>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      /*对应柱状图*/
      GetYear() {
        eventbus.$on('SendPersonalYear', (year) => {
          this.year = year;
          this.$request.post('/api/PersonalData/PersonalMonth', {
            'Date': this.year + '-' + 1,
            "FamilyId":sessionStorage.getItem('familyid')
          }).then((res) => {
            this.incomeData = res['PersonalIncomeMonth'];
            this.spendData = res['PersonalSpendMonth'];
          }).catch();
        })
      },
      /*对应饼图*/
      GetMonth() {
        eventbus.$on('SendPersonalMonth', (month) => {
          this.month = month;
          var dates = this.year + '-' + this.month;
